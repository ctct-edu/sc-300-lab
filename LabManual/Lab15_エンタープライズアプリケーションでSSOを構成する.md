...
5. 受信トレイにある「Salesforce へようこそ: パスワードのリセット」という件名のメールを開き、メール本文にある「アカウントを開始」ボタンをクリックします。
...
### タスク 2 - Salesforceでテストユーザーを作成する

このタスクでは、最後の動作確認で使用するテストユーザーを作成します。

1. 「設定|ホーム」画面の左側ツリーにあるIDを展開し、「ユーザー」をクリックします。「ユーザー」画面に移動したら「新規ユーザー」を作成します。

   ![aad-sso-001](./media/aad-sso-001.BMP)

2. 「新規ユーザー」画面で次の項目を入力し、「保存」をクリックしてテストユーザーを作成します。

   | 項目         | 値                                                           |
   | ------------ | ------------------------------------------------------------ |
   | 姓           | `admin`                                                      |
   | 別名         | `admin`                                                      |
   | メール       | `admin@XXXXXXXXXXX.onmicrosoft.com`  (ラボ 準備で作成したユーザーを指定)<br /><br />※「メール」を入力すると、「ユーザー名」と「ニックネーム」が自動入力されます。 |
   | プロファイル | システム管理者                                               |

> 注: 今回は手順の都合によりプロファイルをシステム管理者としていますが、標準ユーザー等でシングルサインオンを実施することもできます。
