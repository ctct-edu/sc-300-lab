...
# ラボ 14：エンタープライズアプリケーションでSSOを構成する
...
4. 届いたメールを確認します。[https://outlook.live.com/](https://outlook.live.com/)にアクセスし、サインインします。
     ...
     | パスワード                | ラボで使用している Administrative Password と同じ            |
...
1. 「設定|ホーム」画面の左側ツリーにある「ユーザー」を展開し、「ユーザー」をクリックします。

   ![aad-sso-000](./media/aad-sso-000.BMP)

2. 「ユーザー」画面に移動したら「adminXXX@agentforce.com」の行の左にある「編集」をクリックします。

   ![aad-sso-001](./media/aad-sso-001.BMP)

3. 「ユーザー」画面で「ユーザー名」項目を
...
3. 「SAML を使用した統合シングルサインオン」の項目にある「SAMLが有効」のチェックボックスにチェックを入れ、「保存」をクリックします。

     ![aad-sso-016](./media/aad-sso-016.BMP)

     

4. 画面が切り替わり、再び「シングルサインオン設定」画面で「SAMLが有効」にチェックがあれば完了です。画面はこのままにし、次のタスクへ進みます。
...
1. SaleForceの画面に戻ります。画面左側のツリーより「会社の設定」を展開し、「私のドメイン」をクリックします。画面中央にある「現在の [私のドメイン] の URL」に記載のドメインをメモします。
     ...
     > 例：orgfarm-XXXXXXXXXX-dev-ed.develop.my.salesforce.com
...
4. 「SAML シングルサインオン構成」画面の中央にある「Choose File」をクリックし、前の手順でダウンロードした「Salesforce.xml」をアップロードします。アップロード後に「作成」をクリックします。

     ![aad-sso-040](./media/aad-sso-040.BMP)

3. 「SAML シングルサインオン構成」画面に切り替わります。次の項目を入力し「保存」をクリックします。
...
6. 「割り当ての追加」画面で、次の項目を入力し「割り当て」をクリックします。
   ...
   | 項目                     | 値                                    |
   | ------------------------ | ------------------------------------- |
   | ユーザーとグループ       | `admin@XXXXXXXXXXX.onmicrosoft.com　` |
...
7. 「Salesforce | ユーザーとグループ」ブレードの一覧に選択したユーザーが表示されたことを確認します。
   ...
   > 参考:この手順ではEntra IDにあるadminユーザーとSalesforceのユーザーが一致していたため、 ユーザーとグループの追加でSSOを構成することができます。別途ユーザーをSSOさせたい場合、Entra IDユーザーとSalesforceの両方で同一のサインインIDと役割(プロファイル)を構成しないとSSOは成功しません。ユーザーを自動同期させる場合は、別途「プロビジョニング」の設定が必要になります。
   >
...
4. 「IDを検証」画面が出た場合は、admin@XXXXXXXXXXX.onmicrosoft.com のOutlookの画面にアクセスすると「Salesforceで ID を確認」という件名のメールがあるはずです。それに書かれた「確認コード」（6桁の数字）を以下画面の確認コード欄に記入し「検証」をクリックします。

   ![aad-sso-055](./media/aad-sso-055.BMP)

5. Salesforceの画面が表示されました。これでEntraIDユーザーを使用してSalesforceにSSOすることが可能になります。
